...
- **Manual (custom endpoint)**: for self-hosted or relay endpoints that appear in neither the catalog nor a registry — enter the provider name, protocol, base URL, API key, model ID, context size, and thinking support step by step, and the CLI writes the same `providers` / `models` entries as a hand-edited `config.toml`

::: warning
Kimi Code OAuth managed accounts logged in via `/login` do not appear in `/provider`. Use `/login` and `/logout` to manage them.
:::

Catalog and registry imports are also available in non-interactive environments via the shell command: [`kimi provider`](../reference/kimi-command.md#kimi-provider). The manual endpoint flow is TUI-only.
