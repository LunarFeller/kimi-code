...
| `on_effort` | `string` | No | `openai` / `openai_responses` providers only. Effort value sent as `reasoning_effort` when Thinking is switched On for a model with no `support_efforts` declared (e.g. a hand-configured endpoint). Without it, On sends no effort field and the endpoint's server-side default decides whether to reason |
...
`[models."<alias>".overrides]` accepts ordinary model fields such as `max_context_size`, `max_input_size`, `max_output_size`, `capabilities`, `display_name`, `reasoning_key`, `adaptive_thinking`, `support_efforts`, `default_effort`, `on_effort`, and `off_effort`. It does not accept identity / routing fields: `provider`, `model`, `protocol`, `beta_api`, and `base_url`.
