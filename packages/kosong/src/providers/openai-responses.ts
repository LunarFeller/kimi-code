...
export class OpenAIResponsesChatProvider implements ChatProvider {
  ...
  withThinking(effort: ThinkingEffort): OpenAIResponsesChatProvider {
    // 'on' sends the model's declared on value when one is configured and
    // no effort field otherwise; 'off' sends the model's declared off value
    // (e.g. 'none') when one is configured, and omits the field otherwise.
    const reasoningEffort =
      effort === 'off' ? this._offEffort : effort === 'on' ? this._onEffort : effort;
